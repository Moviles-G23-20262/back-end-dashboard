...
function App() {
  ...
  const [query, setQuery] = useState('SELECT * FROM "Material"\nORDER BY "createdAt" DESC\nLIMIT 20;')
  ...
  const runQuery = async () => {
    ...
    setError('')
    try {
      setQueryResult(await executeRawSql({ query }))
    } catch (queryError) {
      setQueryResult(null)
...
function SqlView({ query, setQuery, queryResult, queryRunning, runQuery }: { query: string; setQuery: (query: string) => void; queryResult: SqlQueryResult | null; queryRunning: boolean; runQuery: () => void }) {
